...
const fallbackResponses: Record<string, string> = {
  ...
  pricing: "Our plans: Starter ₹999/month (100 DMs, 2 accounts), Professional ₹2,999/month (1000 DMs, 5 accounts, WhatsApp integration), Enterprise ₹9,999/month (unlimited everything). All plans save 33% and include 14-day free trial + 30-day money-back guarantee. No credit card needed for trial!",
...
function getFallbackResponse(message: string): string {
  ...
  if (lowerMessage.match(/\b(price|pricing|cost|plan|subscription|package)\b/)) {
    return fallbackResponses.pricing
  }
  if (lowerMessage.match(/\b(trial|free|demo|test)\b/)) {
    return fallbackResponses.trial
  }
  if (lowerMessage.match(/\b(feature|capability|what.*do|offer)\b/)) {
  ...
  if (lowerMessage.match(/\b(help|assist|support|need|how)\b/)) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error?.status === 401 || error?.code === 'invalid_api_key') {
      return NextResponse.json({
        success: false,
        response: 'AI service authentication failed. The API key may be invalid.',
        category: 'error'
      })
    }
    ...
    if (error?.code === 'insufficient_quota' || error?.status === 429) {
      // Provide intelligent fallback response when quota is exceeded
      ...
      console.log('⚠️  Using fallback response (quota exceeded)')
